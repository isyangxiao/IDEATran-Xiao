 ...
 *
 * 功能说明:
 * - 气泡翻译: 选中文本后按 Ctrl+Alt+M，翻译结果以气泡形式显示
...
import com.idea.tran.ui.BalloonPanelFactory;
...
public class TranslateAction extends AnAction {
    ...
    public void actionPerformed(@NotNull AnActionEvent e) {
        ...
        if (!service.isReady()) {
            ...
            translateAndShow(e, selectedText, service);
        }
    }
     ...
     */
    private void translateAndShow(AnActionEvent e, String text, TranslationService service) {
        // 自动检测语言方向：如果文本包含中文，则判定为中文→英文
        boolean containsChinese = CHINESE_PATTERN.matcher(text).find();
        String direction = containsChinese ? "中文 → 英文" : "英文 → 中文";
        ...
        new Thread(() -> {
            ...
            com.intellij.openapi.application.ApplicationManager.getApplication().invokeLater(() -> {
                if (result != null && result.isSuccess()) {
                    showTranslationResult(e, text, result.getTextOrNull(), direction);
                } else {
     ...
     * @param direction      翻译方向描述
     */
    private void showTranslationResult(AnActionEvent e, String originalText, String translatedText, String direction) {
        Project project = e.getProject();
        if (project != null) {
            String title = "翻译结果 (" + direction + ")";
            BalloonPanelFactory.showInfo(title, translatedText, project);
        }
    }
}
